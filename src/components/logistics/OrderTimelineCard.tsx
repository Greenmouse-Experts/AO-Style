...
const OrderTimelineCard: React.FC<OrderTimelineProps> = ({
  ...
  const getEventStatus = (eventTimestamp: string | null | undefined, eventType: string): 'completed' | 'current' | 'pending' => {
  ...
  // Add conditional events based on what timestamps exist
  ...
  // Filter out events that don't have timestamps and aren't current/completed
  const visibleEvents = timelineEvents.filter(event =>
    event.timestamp || event.status === 'current' || event.status === 'completed'
  );
      ...
      <div className="mt-6 pt-4 border-t border-gray-100">
        <div className="flex items-center justify-between text-sm">
          <span className="text-gray-600">
            {visibleEvents.filter(e => e.status === 'completed').length} of {timelineEvents.length} steps completed
          </span>
          {currentStatus && (
