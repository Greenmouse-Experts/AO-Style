...
export default function AnkaraGownPage() {
  ...
  const fullBodyMeasurements = [
    { name: "(E + J) - Height", placeholder: "Enter your height" },
    {
      name: "(E + I) - Dress/Gown Length",
  ...
  const handleAddToCartWithFabric = () => {
    ...
    if (!token) {
      ...
      // Retrieve existing pending_cart_data from localStorage
      const existingData = localStorage.getItem("pending_fabric_data");
      let cartData = { items: [] };

      if (existingData) {
        try {
          const parsed = JSON.parse(existingData);
          // If parsed has items array, use it; else, wrap parsed as first item
          if (parsed && Array.isArray(parsed.items)) {
            cartData = parsed;
          } else if (Array.isArray(parsed)) {
            // legacy: if it's an array, wrap as items
      ...
      // Add the new pendingFabric with measurement to the items array
      cartData.items.push({
        ...cartPayload,
        pendingFabricData: { pendingFabric, styleInfo },
        measurements: measurementArr,
      });

      localStorage.setItem("pending_fabric_data", JSON.stringify(cartData));
      toastSuccess("Fabric and measurement saved!");
      localStorage.removeItem("pending_fabric");
      navigate("/view-cart");
      return;
    }
  ...
  const extractDays = (timeString) => {
    ...
    // Pattern 1: "1970-01-01 01:00:00.005 day" -> extract milliseconds as days
    let match = timeString.match(/\.(\d+)/);
    if (match) return parseInt(match[1]);

    // Pattern 2: Look for any number before "day"
    match = timeString.match(/(\d+)\s*day/);
    if (match) return parseInt(match[1]);

    // Pattern 3: Just extract any number from the string
    match = timeString.match(/(\d+)/);
    if (match) return parseInt(match[1]);
                      ...
                      <div className="flex items-center gap-3 bg-green-50 border border-green-200 rounded-lg p-4">
                        ...
                        <div>
                          <p className="text-green-800 font-semibold">
                            You have selected enough fabric for this style!
                          </p>
