...
const Breadcrumb = ({ title, subtitle, just, backgroundImage }) => {
  const routeMap = {
    ...
    "Get Access to the best Tailors and a range of Designs for your next outfit":
      "/products",
  ...
  const subtitleRoute = subtitle
    ? routeMap[subtitle] ||
      routeMap[
        Object.keys(routeMap).find((key) => subtitle.includes(key)) || ""
      ] ||
      "/"
    : "/";
  const justText = just ? just.replace(">", "").trim() : null;
  const justRoute = justText
    ? routeMap[justText] || `/${justText.toLowerCase().replace(/ /g, "-")}`
    : "/";
      ...
      <div className="Resizer Push">
        <div className="relative z-10">
          ...
          <nav className="text-sm text-gray-300 mt-2 flex items-center space-x-2">
            ...
            <span>{">"}</span>
            <Link to={subtitleRoute} className="hover:text-white">
              {subtitle}
            </Link>
            {just && (
              <>
                <span>{">"}</span>
                <Link to={justRoute} className="hover:text-white">
                  {justText}
                </Link>
              </>
            )}
