          ...
          <div className="bg-gradient-to-r from-[#9847FE] to-[#8036D3] px-4 sm:px-8 py-6 border-b border-gray-200">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between space-y-4 sm:space-y-0">
              <div className="flex items-center space-x-4">
                ...
                <div>
                  <h1 className="text-xl sm:text-2xl font-bold text-white">FAQ Management</h1>
          ...
          {faqs && faqs.length > 0 ? (
            <div className="space-y-6">
              <div className="flex items-center justify-between">
                <div>
                  <h3 className="text-lg font-bold text-gray-900">All FAQs</h3>
                  ...
                  <table className="w-full">
                    <thead>
                      <tr className="bg-gradient-to-r from-gray-50 to-gray-100 border-b border-gray-200">
                        <th className="text-left py-4 px-3 sm:px-6 font-bold text-gray-700 text-xs sm:text-sm uppercase tracking-wider w-16">№</th>
                        <th className="text-left py-4 px-3 sm:px-6 font-bold text-gray-700 text-xs sm:text-sm uppercase tracking-wider min-w-48">Question</th>
                        <th className="text-left py-4 px-3 sm:px-6 font-bold text-gray-700 text-xs sm:text-sm uppercase tracking-wider min-w-64 hidden sm:table-cell">Answer</th>
                        <th className="text-left py-4 px-3 sm:px-6 font-bold text-gray-700 text-xs sm:text-sm uppercase tracking-wider w-28">Status</th>
                        <th className="text-left py-4 px-3 sm:px-6 font-bold text-gray-700 text-xs sm:text-sm uppercase tracking-wider w-24">Actions</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-gray-100">
                      {faqs.map((faq, index) => (
                        <tr key={faq.id} className="hover:bg-gray-50/50 transition-all duration-150">
                          <td className="py-4 sm:py-6 px-3 sm:px-6">
                            <div className="flex items-center justify-center w-6 h-6 sm:w-8 sm:h-8 bg-[#9847FE]/10 rounded-lg">
                              <span className="text-xs sm:text-sm font-bold text-[#9847FE]">{index + 1}</span>
                            </div>
                          </td>
                          <td className="py-4 sm:py-6 px-3 sm:px-6">
                            ...
                            >
                              <div
                                className={`w-1.5 h-1.5 sm:w-2 sm:h-2 rounded-full mr-1 sm:mr-2 ${
                                  faq.is_active ? "bg-emerald-500" : "bg-red-500"
                                }`}
                              ></div>
                          ...
                          <td className="py-4 sm:py-6 px-3 sm:px-6">
                            <div className="relative" ref={activeDropdown === faq.id ? dropdownRef : null}>
                              <button
                              ...
                              {activeDropdown === faq.id && (
                                <div className="absolute right-0 top-full mt-2 w-52 bg-white rounded-xl shadow-lg border border-gray-200 z-10 overflow-hidden">
                                  ...
                                  >
                                    ...
                                    <span className="font-medium">Delete FAQ</span>
                                  </button>
                                </div>
                              )}
                            </div>
            ...
            <div className="text-center py-16">
              <div className="max-w-md mx-auto px-4">
                ...
                <p className="text-gray-500 mb-6 leading-relaxed text-sm sm:text-base">
                  You haven't created any frequently asked questions yet. Start by adding your first FAQ to help your users.
                </p>
                <button
                  onClick={() => setShowForm(true)}
