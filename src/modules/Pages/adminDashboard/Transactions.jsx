...
const PaymentTransactionTable = () => {
    ...
    const columns = [
            ...
            label: () => (
                <input
                    ...
                    onChange={(e) => {
                        ...
                        const newSelected = new Set(newSelectAll ? data.map(item => item.id) : []);
        ...
        { label: "Transaction Type", key: "transactionType", className: "text-gray-500 font-medium text-sm py-4" },
        {
            ...
            render: (status) => (
                <span
                    className={`px-2 py-1 text-sm rounded-full font-medium ${status === "In-Progress"
                ...
                <div className="relative" ref={dropdownRef}>
                    ...
                    {openDropdown === row.id && (
                        <div className="absolute right-0 mt-2 w-40 bg-white rounded-md z-10 shadow-lg">
                            ...
                            <button className="block px-4 py-2 text-gray-700 hover:bg-gray-100 w-full">Edit Transaction</button>
                            <button className="block px-4 py-2 text-red-500 hover:bg-red-100 w-full">Remove Transaction</button>
                        </div>
                    )}
                </div>
            ),
    ...
    const filteredData = data.filter((transaction) => {
        ...
        const matchesTab = activeTab === "All Transactions" ||
            (activeTab === "Income" && ["Payment"].includes(transaction.transactionType)) ||
            (activeTab === "Payouts" && ["Payout", "Refund"].includes(transaction.transactionType));
