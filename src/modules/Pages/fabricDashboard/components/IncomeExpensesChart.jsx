...
import useVendorMonthlyMetrics from "../../../../hooks/analytics/useGetMonthlyMetrics";
import {
  CustomTooltip,
  months,
} from "../../adminDashboard/components/IncomeExpensesChart";
...
const IncomeExpensesChart = () => {
  const {
    ...
    isError,
    data: vendorMetrics,
  } = useVendorMonthlyMetrics();

  let chartData = [];
  if (Array.isArray(vendorMetrics?.data) && vendorMetrics?.data.length === 12) {
    chartData = vendorMetrics?.data.map((income, idx) => ({
      month: months[idx],
      income: income?.fabric,
    }));
  }
