...
export default function OrderRequests() {
  ...
  const getPickupContact = (item: Order) => {
    if (isSecondLeg(item)) {
      ...
      return {
        email: item?.order_items?.[1]?.product?.creator?.name || "tailor@example.com",
        phone: item?.order_items?.[1]?.product?.creator?.phone  || "N/A",
        type: "Tailor"
      };
    } else {
      // First leg: Show fabric vendor info
      return {
        email: item?.order_items?.[0]?.product?.creator?.name  || "fabricvendor@example.com",
        phone: item?.order_items?.[0]?.product?.creator?.phone  || "N/A",
        type: "Fabric Vendor"
      };
    }
  };

  // Helper function to get timestamp based on status
  const getTimestampByStatus = (item: Order) => {
    const status = item.status.toUpperCase();
    
    switch (status) {
      case "DISPATCHED_TO_AGENT":
        return item.dispatched_to_agent_at;
      case "IN_TRANSIT":
        return item.in_transit_at;
      case "OUT_FOR_DELIVERY":
        return item.out_for_delivery_at;
      case "DELIVERED":
        return item.delivered_at;
      case "DELIVERED_TO_TAILOR":
        return item.delivered_to_tailor_at;
      case "CANCELLED":
  ...
  const columns: {
  ...
  }[] = [
    {
      ...
      render: (value: any, item: Order) => {
        ...
        const timestamp = getTimestampByStatus(item);
    ...
    },
    {
      ...
      render: (value: any, item: Order) => {
        // Clicking the span will toggle showing the full address in an alert or modal. We'll use alert for simplicity.
        const fullAddress = addressToDisplay(item);
        return (
          <div className="flex items-center gap-2 max-w-[200px]">
            <MapPin className="w-4 h-4 text-base-content/60 flex-shrink-0" />
            <span
              className="truncate text-sm bg-base-300 rounded px-2 py-1 cursor-pointer select-text"
              title={fullAddress}
              onClick={(e) => {
                e.stopPropagation();
  ...
  const addressToDisplay = (item: Order) =>{
    if(isSecondLeg(item)) {
      ...
      return item?.order_items?.[1]?.product?.creator?.profile?.address|| "Tailor address not available"
    } else{
      // First leg: Show fabric vendor address (pickup location)
      return  item?.order_items?.[0]?.product?.creator?.profile?.address || "Fabric vendor address not available"
    }
  }
