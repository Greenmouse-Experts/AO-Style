...
export default function InboxPage() {
  ...
  useEffect(() => {
    if (profileSuccess && profileData) {
      console.log("=== LOGISTICS USER PROFILE LOADED ===");
      ...
      console.error("=== LOGISTICS PROFILE LOADING ERROR ===");
      ...
      console.log("=== LOGISTICS PROFILE LOADING ===");
  ...
  useEffect(() => {
    console.log("=== INITIALIZING LOGISTICS SOCKET CONNECTION ===");
    ...
    if (userToken && userId && !profileLoading) {
      ...
      socketInstance.on("disconnect", (reason) => {
        console.log("=== LOGISTICS SOCKET DISCONNECTED ===");
      ...
      socketInstance.on("chatsRetrieved", (data) => {
        console.log("=== LOGISTICS CHATS RETRIEVED ON LOAD ===");
      ...
      if (userId) {
        ...
        socketInstance.on(`chatsRetrieved:${userId}`, (data) => {
          console.log(`=== LOGISTICS USER-SPECIFIC CHATS RETRIEVED (${userId}) ===`);
          console.log("Full response:", JSON.stringify(data, null, 2));
          console.log("Status:", data?.status);
          console.log("Message:", data?.message);
          console.log("Result array:", data?.data?.result);
        ...
        socketInstance.on(`messagesRetrieved:${userId}`, (data) => {
          console.log(`=== LOGISTICS USER-SPECIFIC MESSAGES RETRIEVED (${userId}) ===`);
          console.log("Full response:", JSON.stringify(data, null, 2));
          console.log("Status:", data?.status);
        ...
        const setupChatSpecificListener = (chatId) => {
          ...
          socketInstance.on(eventName, (data) => {
            console.log(`=== LOGISTICS CHAT-SPECIFIC MESSAGES RETRIEVED (${chatId}:${userId}) ===`);
            console.log("Full response:", JSON.stringify(data, null, 2));
            console.log("Status:", data?.status);
            console.log("Messages array:", data?.data?.result);
            console.log("Selected chat from ref:", selectedChatRef.current);
        ...
        socketInstance.on(`recentChatRetrieved:${userId}`, (data) => {
          ...
          if (data?.data) {
            ...
            if (currentSelectedChat && currentSelectedChat.id === data.data.id) {
              console.log("🔄 Auto-refreshing messages for currently selected logistics chat (user-specific)");
      ...
      socketInstance.on("messagesRetrieved", (data) => {
        console.log("=== LOGISTICS MESSAGES RETRIEVED ===");
        console.log("Full response:", JSON.stringify(data, null, 2));
        console.log("Status:", data?.status);
        console.log("Messages array:", data?.data?.result);
        console.log("Selected chat from ref:", selectedChatRef.current);
      ...
      socketInstance.on("recentChatRetrieved", (data) => {
        console.log("=== LOGISTICS RECENT CHAT RETRIEVED ===");
        console.log("Chat data:", JSON.stringify(data, null, 2));
        console.log("======================================");
        ...
        if (data?.data) {
          ...
          // Auto-refresh messages if this chat is currently selected
          if (currentSelectedChat && currentSelectedChat.id === data.data.id) {
            console.log("🔄 Auto-refreshing messages for currently selected logistics chat");
            socketInstance.emit("retrieveMessages", {
              token: userToken,
              chatBuddy: currentSelectedChat.chat_buddy.id,
            });
      ...
      socketInstance.on("connect_error", (error) => {
        console.error("=== LOGISTICS SOCKET CONNECTION ERROR ===");
      ...
      return () => {
        console.log("=== CLEANING UP LOGISTICS SOCKET ===");
        console.log("User ID:", userId);
        socketInstance.disconnect();
        console.log("===================================");
      };
    } else {
      console.log("=== WAITING FOR LOGISTICS USER PROFILE OR TOKEN ===");
  ...
  const sendMessage = () => {
    ...
    console.log("📤 === SENDING LOGISTICS MESSAGE ===");
  ...
  const handleChatClick = (chat) => {
    console.log("🔄 Logistics chat selected:", chat);
    ...
    return (
      <div className="flex items-center justify-center h-screen bg-gray-50">
        <div className="text-center">
          ...
          <p className="text-gray-600">Loading logistics user profile...</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="flex items-center justify-center h-screen bg-gray-50">
        <div className="text-center">
          ...
          <p className="text-gray-600 mb-4">Failed to load logistics user profile</p>
          <button 
