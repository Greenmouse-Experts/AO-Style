...
export default function FabricVendorPage() {
                ...
                <div className="relative" ref={dropdownRef}>
                    ...
                    {openDropdown === row.id && (
                        <div className="absolute right-0 mt-2 w-40 bg-white rounded-md z-10">
                            <button className="block px-4 py-2 text-gray-700 hover:bg-gray-100 w-full">View Fabric Vendor</button>
                            <button className="block px-4 py-2 text-gray-700 hover:bg-gray-100 w-full">Edit  Fabric Vendor</button>
                            <button className="block px-4 py-2 text-red-500 hover:bg-red-100 w-full">Remove Fabric Vendor</button>
                        </div>
                    )}
                </div>
            ),
    ...
    const filteredVendors = vendors.filter((vendor) => {
        if (filter !== "All Fabric Vendors" && vendor.status !== filter) return false;
        ...
        <div>
            <div className="bg-white px-4 sm:px-6 py-4 mb-6 relative">
                ...
                <p className="text-gray-500 mt-2 text-sm sm:text-base">
                    <Link to="/sales" className="text-blue-500 hover:underline">Dashboard</Link> &gt; Fabric Vendors
                </p>
            </div>
            ...
            <div className="bg-white p-4 rounded-lg overflow-x-auto">
                <div className="flex flex-wrap justify-between items-center pb-3 mb-4 gap-4">
                    ...
                    <div className="flex flex-wrap gap-4 sm:gap-6 text-gray-600 text-sm font-medium w-full sm:w-auto">
                        {["All Fabric Vendors", "Approved", "Pending", "Declined"].map((tab) => (
                            <button
                                key={tab}
                                onClick={() => setFilter(tab)}
                                className={`font-medium ${filter === tab ? "text-[#A14DF6] border-b-2 border-[#A14DF6]" : "text-gray-500"}`}
                            >
