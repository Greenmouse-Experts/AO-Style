            ...
            <div className="bg-white p-3 md:p-4 rounded-lg">
                <div className="flex flex-col md:flex-row md:justify-between md:items-center pb-3 mb-4 gap-3 md:gap-4">
                    <div className="flex flex-wrap gap-2 md:gap-0 md:space-x-4 lg:space-x-6 text-gray-600 text-xs md:text-sm font-medium overflow-x-auto">
                        <button onClick={() => setFilter("all")} className={`whitespace-nowrap font-medium pb-2 ${filter === "all" ? "text-[#A14DF6] border-b-2 border-[#A14DF6]" : "text-gray-500"}`}>
                            All Notification
                        </button>
                        <button onClick={() => setFilter("read")} className={`whitespace-nowrap font-medium pb-2 ${filter === "read" ? "text-[#A14DF6] border-b-2 border-[#A14DF6]" : "text-gray-500"}`}>
                            All Read (30)
                        </button>
                        <button onClick={() => setFilter("unread")} className={`whitespace-nowrap font-medium pb-2 ${filter === "unread" ? "text-[#A14DF6] border-b-2 border-[#A14DF6]" : "text-gray-500"}`}>
                            All Unread (60)
                        </button>
                    </div>
                    <div className="flex flex-col sm:flex-row gap-2 w-full md:w-auto">
                        ...
                        <button className="px-3 md:px-4 py-2 bg-gray-200 rounded-md text-xs md:text-sm whitespace-nowrap">Export As ▼</button>
                        <button className="px-3 md:px-4 py-2 bg-gray-200 rounded-md text-xs md:text-sm whitespace-nowrap">Sort: Newest First ▼</button>
                    </div>
                </div>

                <div>
                    {filteredNotifications.map((notification) => (
                        <div key={notification.id} className={`flex flex-col sm:flex-row sm:items-center p-3 md:p-5 gap-2 md:gap-3 cursor-pointer transition-colors hover:bg-gray-50 ${notification.unread ? "bg-purple-100" : ""}`} onClick={() => toggleExpand(notification.id)}>
                            <img src="https://res.cloudinary.com/greenmouse-tech/image/upload/v1741985895/AoStyle/image_cuxdyt.png" className="w-10 h-10 md:w-14 md:h-14 flex-shrink-0" alt="icon" />
                            <div className="flex-1 min-w-0">
                                ...
                                <div className="flex items-center gap-2 mt-1 md:mt-0">
                                    ...
                                    {notification.unread && <span className="h-2 w-2 bg-[#A14DF6] rounded-full flex-shrink-0"></span>}
                                </div>
                            </div>
                        </div>
                    ))}
